...
The RI requires you to register the `OutOfCreditException` and the `ProblemDetailHandler` (see the `ri` README for details). And you have to unwrap the JAX-RS `ResponseProcessingException` exception before you can catch the `OutOfCreditException`.
